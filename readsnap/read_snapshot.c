...
int do_what_you_want(double *array_a, double *array_s)
{
   ...
   //printf("kb = %f number of kb = %d\n", kbins[b], count[b]);
    ...
    ret = strrchr(output, '/');
    len = ret - output;
    strncpy(output1, output, len+1);
    strncpy(output2, output, len+1);
    strncpy(output3, output, len+1);
    strcat(output1, "Rui_data.txt");
    strcat(output2, "Rui_data_log.txt");
    strcat(output3, "Rui_nor.txt");
    nor = fopen(output3, "w");
    fprintf(nor, "%f", pk_nor);
    fclose(nor);
    t = fopen(output1, "w");
    tlog = fopen(output2, "w");
