    ...
    type CAMBparams
        ...
        logical input_Allxi_on !Rui input or calculate
    ...
    do nu_i = 1, CP%Nu_mass_eigenstates
        ...
        print*, "nu_mass_degeneracy ", CP%Nu_mass_degeneracies(nu_i)   !zzc
        print*, "grhomass ", grhormass(nu_i)   !zzc
    end do
    ...
    if (.not.call_again) then
        !Rui: decide whether need to calculate
        if (.not. CP%input_Allxi_on) then
            !terry: calculate CP%xi_nu(1) CP%xi_nu(2) from CP%xi_nu(3)  zzc
            ...
            call init_massive_nu(CP%omegan /=0, CP%xi_nu) !terry zzc
    ...
    if (present(error)) then
        ...
        write(*,'("Neutrino degeneracy  = ",f9.6)') CP%xi_nu !Kenny: edited terry zzc
...
500     if (L< lmax_extrap_highl) &
    ...
    real(dl) :: nu_q(nqmax0), nu_int_kernel(nqmax0, 3), nu_w(nqmax0)  !zzc

    integer nqmax !actual number of q modes evolves
    real(dl) :: xi_fac(2,3) !terry

    public const,Nu_Init,Nu_background, Nu_rho, Nu_drho,  nqmax0, nqmax, &
        nu_int_kernel, nu_q, sum_mnu_for_m1, neutrino_mass_fac, GetOmegaNu, nuRhoPres
    contains
    ...
    real(dl), intent(in) :: xi_camb(3)      !zzc
    ...
    do i=1, CP%Nu_mass_eigenstates
        ...
        print *, 'mass fraction', CP%Nu_mass_fractions(i)
        print *, 'mass degeneracy', CP%Nu_mass_degeneracies(i)
    end do
    ...
    if (allocated(r1) .or. allocated(p1) .or. allocated(dr1) .or. allocated(dp1) .or. allocated(ddr1)) then !Kenny: edited terry
        ...
        deallocate(p1)                                                                                        !Kenny: edited
        deallocate(dr1)                                                                                       !Kenny: edited
        deallocate(dp1)                                                                                       !Kenny: edited
        deallocate(ddr1)                                                                                      !Kenny: edited
    end if                                                                                                  !Kenny: edited
    allocate(r1(nrhopn,3),p1(nrhopn,3),dr1(nrhopn,3),dp1(nrhopn,3),ddr1(nrhopn,3))                           !Kenny: edited terry zzc
    ...
    if (nqmax==3) then
        ...
        nu_w(1:3) = (/0.776251, 0.109268, 0.00249701/)!terry
        !nu_int_kernel(1:3, 1) = (/0.0687359, 3.31435, 2.29911/)
        !nu_int_kernel(1:3, 2) = (/0.0687359, 3.31435, 2.29911/)
        !nu_int_kernel(1:3, 3) = (/0.0687359, 3.31435, 2.29911/)
    else if (nqmax==4) then
        !This seems to be very accurate (limited by other numerics)
        nu_q(1:4) = (/0.7, 2.62814, 5.90428, 12.0/)
        nu_w(1:4) = (/0.747213, 0.177875, 0.0116737, 0.0000558314/)!terry
        !nu_int_kernel(1:4,1) = (/0.0200251, 1.84539, 3.52736, 0.289427/)
        !nu_int_kernel(1:4,2) = (/0.0200251, 1.84539, 3.52736, 0.289427/)
        !nu_int_kernel(1:4,3) = (/0.0200251, 1.84539, 3.52736, 0.289427/)
    else if (nqmax==5) then
        !exact for n=-4,-2..3
        !This seems to be very accurate (limited by other numerics)
        nu_q(1:5) = (/0.583165, 2.0, 4.0, 7.26582, 13.0/)
        nu_w(1:5) = (/0.681808, 0.222159, 0.0454693, 0.00294858, 0.0000177609/)!terry
        !nu_int_kernel(1:5,1) = (/0.0081201, 0.689407, 2.8063, 2.05156, 0.126817/)
        !nu_int_kernel(1:5,2) = (/0.0081201, 0.689407, 2.8063, 2.05156, 0.126817/)
        !nu_int_kernel(1:5,3) = (/0.0081201, 0.689407, 2.8063, 2.05156, 0.126817/)
    else
        ...
        do i=1,nqmax
            ...
            nu_int_kernel(i, 1:3)=dq*q**3/(exp(q)+1._dl) * (-0.25_dl*dlfdlq) !now evolve 4F_l/dlfdlq(i) zzc
        end do
    end if

    if (nqmax>=3 .and. nqmax<=5) then !terry
    !	    nqmax=8
    !	    nu_q(1:8) = (/0.17028, 0.903702, 2.25109, 4.2667, 7.04591, 10.7585, 15.7407, 22.8631/)
    !	    nu_w(1:8) = (/0.366082,0.421494,0.175147,0.0334546,0.00278069,0.0000919391,7.90956e-7,2.10538e-9/)!terry
        do i = 1, nqmax
            nu_int_kernel(i, :) = nu_w(i) * exp(nu_q(i)) * nu_q(i)**4 * exp(nu_q(i)-xi_camb(:)) &
            / (1.d0 + exp(nu_q(i)-xi_camb(:)))**2 / 4 !terry
            nu_int_kernel(i, :) = nu_int_kernel(i,:) + nu_w(i) * exp(nu_q(i)) * nu_q(i)**4 * exp(nu_q(i)+xi_camb(:)) &
                / (1.d0 + exp(nu_q(i)+xi_camb(:)))**2 / 4
            nu_int_kernel(i,:) = nu_int_kernel(i,:)/2
            !print *, 'test nu int kernel', nu_int_kernel(i)
        enddo
    endif !terry
    ...
    call nuRhoPres(am,rhonu,pnu, xi_camb(1))        !zzc
    r1(i,1)=log(rhonu)
    p1(i,1)=log(pnu)

    call nuRhoPres(am,rhonu,pnu, xi_camb(2))
    r1(i,2)=log(rhonu)
    p1(i,2)=log(pnu)

    call nuRhoPres(am,rhonu,pnu, xi_camb(3))
    r1(i,3)=log(rhonu)
    p1(i,3)=log(pnu)
    ...
    do i = 1, 3!terry zzc
        ...
        call splder(r1(:,i),dr1(:,i),nrhopn,spline_data) !terry
        call splder(p1(:,i),dp1(:,i),nrhopn,spline_data) !terry
        call splder(dr1(:,i),ddr1(:,i),nrhopn,spline_data) !terry
    enddo !terry
    ...
    !cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
    subroutine nuRhoPres(am,rhonu,pnu, xi_camb)
    ...
    real(dl) dum1(nq+1),dum2(nq+1), dum1_anti(nq+1),dum2_anti(nq+1)
    real(dl), intent(in) :: am
    real(dl), intent(out) ::  rhonu,pnu
    real(dl), intent(in) :: xi_camb !zzc
    integer i
    real(dl) q,aq,v,aqdn,adq,aqdn_anti
    real(dl) rhonu_anti, pnu_anti            !Kenny: edited zzc
    ...
    dum2_anti(1)=0._dl !Kenny: edited   zzc

    do  i=1,nq
        q=i*adq
        aq=am/q
        v=1._dl/sqrt(1._dl+aq*aq)
        aqdn=adq*q*q*q/(exp(q-xi_camb)+1._dl)       !zzc
        aqdn_anti=adq*q*q*q/(exp(q+xi_camb)+1._dl)
        dum1(i+1)=aqdn/v
        dum2(i+1)=aqdn*v
    ...
    call splint(dum1_anti,rhonu_anti,nq+1)
    call splint(dum2_anti,pnu_anti,nq+1)
    !  Apply asymptotic corrrection for q>qmax and normalize by relativistic
    !  energy density.

    rhonu=(rhonu+dum1(nq+1)/adq)/const
    rhonu_anti=(rhonu_anti+dum1_anti(nq+1)/adq)/const       !Kenny: edited  zzc
    pnu=(pnu+dum2(nq+1)/adq)/const/3._dl
    pnu_anti=(pnu_anti+dum2_anti(nq+1)/adq)/const/3._dl     !Kenny: edited  zzc

    rhonu=(rhonu+rhonu_anti)/2._dl  !Kenny: edited   zzc
    pnu=(pnu+pnu_anti)/2._dl        !Kenny: edited   zzc

    end subroutine nuRhoPres

    !cccccccccccccccccccccccccccccccccccccccccc
    subroutine Nu_background(am,rhonu,pnu, nu_i)    !zzc
    use precision
    use ModelParams
    use constants
    real(dl), intent(in) :: am
    integer, intent(in) :: nu_i !terry zzc
    real(dl), intent(out) :: rhonu, pnu
    logical, save :: printed = .false.  !??zzc
    ...
    if (am <= am_minp) then
        ...
        !pnu=(2-rhonu)/3._dl
        pnu=(2 * xi_fac(1,nu_i) - rhonu)/3._dl!terry
    if (.not. printed) then !terry
        !print *, 'am <= am_minp Nu_background', am
        printed = .true.
    endif
        return

    else if (am >= am_maxp .and. CP%xi_nu(nu_i)/=0.d0) then
    !else if (am >= am_maxp) then
        print *, 'currently not supported to use asymptotic behavior for am >= am_maxp in Nu_background'
        stop
    else if (am >= am_maxp) then
    ...
    rhonu=r1(i,nu_i)+d*(dr1(i,nu_i)+d*(3._dl*(r1(i+1,nu_i)-r1(i,nu_i))-2._dl*dr1(i,nu_i) &
    -dr1(i+1,nu_i)+d*(dr1(i,nu_i)+dr1(i+1,nu_i)+2._dl*(r1(i,nu_i)-r1(i+1,nu_i)))))
    pnu=p1(i,nu_i)+d*(dp1(i,nu_i)+d*(3._dl*(p1(i+1,nu_i)-p1(i,nu_i))-2._dl*dp1(i,nu_i) &
    -dp1(i+1,nu_i)+d*(dp1(i,nu_i)+dp1(i+1,nu_i)+2._dl*(p1(i,nu_i)-p1(i+1,nu_i)))))
    rhonu=exp(rhonu)
    pnu=exp(pnu)
    ...
    !cccccccccccccccccccccccccccccccccccccccccc
    subroutine Nu_rho(am,rhonu, nu_i)   !zzc
    use precision
    use ModelParams
    use constants
    integer, intent(in) :: nu_i !terry  zzc
    ...
    if (am <= am_minp) then
        !rhonu=1._dl + const2*am**2
        rhonu = xi_fac(1,nu_i) + const2*am**2 * xi_fac(2,nu_i) !terry zzc
    if (.not. printed) then
        !print *, 'am <= am_minp Nu_rho', am
        printed = .true.
    endif
        return

    else if (am >= am_maxp .and. CP%xi_nu(nu_i)/=0.d0) then
        print *, 'currently not supported to use asymptotic behavior for am >= am_maxp in Nu_rho'
        stop
    ...
    function Nu_drho(am,adotoa,rhonu,nu_i) result (rhonudot)
    use precision
    use ModelParams
    use constants
    ...
    if (am< am_minp) then
        ...
        rhonudot = rhonudot * xi_fac(2,nu_i)    !zzc

    else if (am>am_maxp .and. CP%xi_nu(nu_i)/=0.d0) then
        print *, 'currently not supported to use asymptotic behavior for am >= am_maxp in Nu_drho'
        stop
        ...
        rhonudot=dr1(i,nu_i)+d*(ddr1(i,nu_i)+d*(3._dl*(dr1(i+1,nu_i)-dr1(i,nu_i)) &
        -2._dl*ddr1(i,nu_i)-ddr1(i+1,nu_i)+d*(ddr1(i,nu_i)+ddr1(i+1,nu_i) &
        +2._dl*(dr1(i,nu_i)-dr1(i+1,nu_i)))))
    ...
    subroutine init_massive_nu(has_massive_nu, xi_camb)
    use MassiveNu
    use ModelParams
    implicit none
    ...
    if (has_massive_nu .or. any(xi_camb /= 0.d0)) then
        call Nu_Init(xi_camb)
    else
        nu_masses = 0
    end if
    ...
    if (CP%AccurateReionization .and. FeedbackLevel > 0 .and. CP%DerivedParameters) then
        print *, 'error flag = ',  global_error_flag
