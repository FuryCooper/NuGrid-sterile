    ...
    GetOmegak = 0.0 !zzc: force it to be 0
    ...
    use MassiveNu
    use LambdaGeneral
    use constants !terry    zzc
    implicit none
    real(dl) dtauda
    real(dl), intent(IN) :: a
    real(dl) rhonu,grhoa2, a2
    integer nu_i
    real(dl) rhonu_m_a_massless(3) !terry   zzc
    integer int_nu_massless     !zzc


    a2=a**2

    !  8*pi*G*rho*a**4.
    !print*, "in equations.f90 grhok= ",  grhok  !zzc

    grhoa2=grhok*a2+(grhoc+grhob)*a+grhog !Kenny: edited terry
    !int_nu_massless = int(CP%Num_Nu_massless)
    !if (int_nu_massless>0) then
    !    grhoa2=grhoa2+sum(rhonu_m_a_massless(1:int_nu_massless))/int_nu_massless*grhornomass !Kenny: edited terry
    !endif       !zzc
    ...
    if (CP%Num_Nu_massive /= 0) then
        ...
        do nu_i = 1, CP%nu_mass_eigenstates
            call Nu_rho(a*nu_masses(nu_i),rhonu,nu_i)   !zzc
            !call Nu_rho(a*0,rhonu,nu_i)   !zzc
    ...
    call Nu_background(a*nu_masses(nu_i),rhonu,pnu, nu_i)   !zzc
    ...
    do nu_i = 1, CP%Nu_mass_eigenstates
        ...
        if (EV%MassiveNuApprox(nu_i)) then
            ...
            pinu=pinu/rhonu
            adotoa = 1/(a*dtauda(a))
            rhonudot = Nu_drho(a*nu_masses(nu_i),adotoa,rhonu, nu_i)    !zzc
    ...
    do iq=1,EV%nq(nu_i)
        ...
        drhonu=drhonu+ nu_int_kernel(iq, nu_i)* y(ind)/v    !zzc
        fnu=fnu+nu_int_kernel(iq, nu_i)* y(ind+1)   !zzc
        if (present(dpnu)) then
            dpnu=dpnu+  nu_int_kernel(iq, nu_i)* y(ind)*v !zzc
            pinu=pinu+ nu_int_kernel(iq, nu_i)*y(ind+2)*v !zzc
        end if
    ...
    do iq=EV%nq(nu_i)+1,nqmax
        ...
        tmp = nu_int_kernel(iq, nu_i)*(y(EV%r_ix)  + pert_scale*y(ind)  ) !zzc
        drhonu=drhonu+ tmp/v
        fnu=fnu+nu_int_kernel(iq, nu_i)*(y(EV%r_ix+1)+ pert_scale*y(ind+1))   !zzc
        if (present(dpnu)) then
            dpnu=dpnu+ tmp*v
            pinu = pinu+ nu_int_kernel(iq, nu_i)*(y(EV%r_ix+2)+ pert_scale*y(ind+2))*v  !zzc
        end if
    end do
    ...
    do iq=1,EV%nq(nu_i)
        ...
        pinudot=pinudot+nu_int_kernel(iq, nu_i)*(ydot(ind)*v+y(ind)*vdot) !zzc
        ind=ind+EV%lmaxnu_tau(nu_i)+1
    end do
    ind = EV%nu_pert_ix+2
    do iq=EV%nq(nu_i)+1,nqmax
        ...
        pinudot=pinudot+nu_int_kernel(iq,nu_i)*(psi2dot*v+psi2*vdot)    !zzc
    end do
    ...
    do iq=1, EV%nq(nu_i)
        ...
        aq=am/q
        v=1._dl/sqrt(1._dl+aq*aq)
        pinu=pinu+nu_int_kernel(iq, nu_i)*y(ind)*v  !zzc
    ...
    do iq=1, EV%nq(nu_i)
        ...
        aq=am/q
        v=1._dl/sqrt(1._dl+aq*aq)
        G11=G11+nu_int_kernel(iq, nu_i)*y(ind+1)*v**2   !zzc
        if (EV%lmaxnu_tau(nu_i)>2) then
            G30=G30+nu_int_kernel(iq, nu_i)*y(ind+3)*v**2   !zzc
        end if
        ind = ind+EV%lmaxnu_tau(nu_i)+1
    end do
