...
void begrun(void)
{
  ...
  if (All.expan_on == 0)
  {
    All.Omega_nu0_expan = neutrino_integration(1.0, 0.0, 0.0) * All.NNeutrino;
  }
  if (All.expan_on == 1)
  {
  ...
  All.Omega_nu0_frstr = neutrino_integration(1.0, 0.0 ,0.0) * All.NNeutrino;
  ...
  printf("Number Density: numden_0=%f ", All.numdens0);
  for (int i = 0; i < All.NNeutrino; i++)
  {
    printf("NumDens%d=%f ", i, All.NumDens[i]);
  }
  //step_index = 1;

  allocate_commbuffers();	/* ... allocate buffer-memory for particle exchange during force computation */
  set_units();
