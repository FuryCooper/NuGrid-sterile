...
int main(int argc, char **argv)
{
  ...
  if(argc < 2)
	...
	if (expan_on == 0)
	{
		Omega_nu0_expan = neutrino_integration(1.0, 0.0, 0.0) * NNeutrino;
	}
	...
	Omega_nu0_frstr = neutrino_integration(1.0, 0.0, 0.0) * NNeutrino;
...
void displacement_fields(void)
{
	...
	roneu =	0.0;
    if(expan_on == 0){
        roneu = neutrino_integration(InitTime, 0., 0.) * NNeutrino;
	}
    if(expan_on == 1){
		for (int i = 0; i < NNeutrino; i++)
		{
			roneu += neutrino_integration(InitTime, Mass[i], Xi[i]);
		}
    }
