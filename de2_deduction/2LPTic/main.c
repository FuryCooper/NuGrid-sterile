...
int main(int argc, char **argv)
{
  ...
  read_parameterfile(argv[1]);
	...
	//two types active/sterile neutrinos
	if(lepton_asymmetry == 3){
    xi_2 = 0;
    xi_1 = 0.1;
    }

	//only for one active/sterile neturino
    if(lepton_asymmetry == 2){
    xi_2 = 0;
    xi_1 = 0;
    }

    if(lepton_asymmetry == 1){
    xi_2 = cal_xi2(xi_3);
    xi_1 = cal_xi1(xi_2, xi_3);
    }
    
    if(lepton_asymmetry == 0){
        xi_2 = xi_3;
        xi_1 = xi_3;
    }
    
    printf("here are the xis xi1 %f xi2 %f xi3 %f\n", xi_1, xi_2, xi_3);
    
    if(mass_hierarchy == 0){
        //normal
        mass_2 = sqrt(mass_1*mass_1 + 7.59e-5);
        mass_3 = sqrt(mass_2*mass_2 + 2.32e-3);
    }
    
    if(mass_hierarchy == 1){
        //inverted
        mass_2 = sqrt(mass_1*mass_1 + 2.32e-3);
        mass_3 = sqrt(mass_2*mass_2 + 7.59e-5);
    }
	...
	//only for one active/sterile neutrino
    if(mass_hierarchy == 3){
	mass_2 = 0;
	mass_3 = mass_1;
	mass_1 = 0;
    ...
    printf("here are the masses hierarchy %d m1 %f m2 %f m3 %f\n", mass_hierarchy, mass_1, mass_2, mass_3);
	
	//only for one active + sterile neutrino
	if(expan_on == 3){
		Omega_nu0_expan = neutrino_integration(1.0, mass_1, xi_1) + neutrino_integration(1.0, mass_3, xi_3);
	}
	//only for one active/sterile neutrino
    if(expan_on == 2){
	Omega_nu0_expan = neutrino_integration(1.0, mass_3, xi_3);
    }
 
    if(expan_on == 1){
        Omega_nu0_expan = neutrino_integration(1.0, mass_1, xi_1) + neutrino_integration(1.0, mass_2, xi_2) + neutrino_integration(1.0, mass_3, xi_3);
    }
    
    if(expan_on == 0){
        Omega_nu0_expan = neutrino_integration(1.0, 0., 0.) * 3;
    }
    
    //Omega_nu0_frstr = neutrino_integration(1.0, mass_1, xi_1) + neutrino_integration(1.0, mass_2, xi_2) + neutrino_integration(1.0, mass_3, xi_3);
    //only for one active/sterile neutrino and for one active + sterile neutrino
	switch (expan_on)
	{
	case 3:
		Omega_nu0_frstr = neutrino_integration(1.0, 0., 0.) * 2;
		break;

	case 2:
		Omega_nu0_frstr = neutrino_integration(1.0, 0., 0.);
		break;
	
	default:
		Omega_nu0_frstr = neutrino_integration(1.0, 0., 0.) * 3;
		break;
	}
...
void displacement_fields(void)
{
  ...
  if(ThisTask == 0)
    ...
    if(expan_on == 0){
        roneu = neutrino_integration(InitTime, 0., 0.) * 3;
    }
    
    if(expan_on == 1){
        roneu = neutrino_integration(InitTime, mass_1, xi_1) + neutrino_integration(InitTime, mass_2, xi_2) + neutrino_integration(InitTime, mass_3, xi_3);
    }

    //only one active/sterile neutrino
    if(expan_on == 2){
	roneu = neutrino_integration(InitTime, mass_3, xi_3);
    }
    ...
    hubble_a =
    Hubble * sqrt(Omega / pow(InitTime, 3) + (1 - Omega - OmegaLambda - Omega_nu0_expan) / pow(InitTime, 2) + OmegaLambda + roneu);
